...
def main():
    parser = argparse.ArgumentParser(
        description="Compare texts to find common word sequences (n-grams)"
    )

    parser.add_argument(
        "files", nargs="+", type=argparse.FileType("rU"), help="text files to compare"
    )
    ...
    parser.add_argument(
        ...
        help="min number of words to look for (default: {})".format(DEFAULT_MIN_WORDS),
    )
    ...
    parser.add_argument(
        "--zero-results",
        type=int,
        default=DEFAULT_ZERO_RESULTS,
        help="""stop comparison after this many zero results for
                             next n-gram length, set to 0 to disable (default: {})""".format(
            DEFAULT_ZERO_RESULTS
        ),
    )
    ...
    parser.add_argument(
        ...
        help="""what characters should count as punctuation (default: {})""".format(
            DEFAULT_PUNCTUATION
        ),
    )
    ...
    parser.add_argument(
        "--include-subgrams",
        action="store_true",
        default=DEFAULT_INCLUDE_SUBGRAMS,
        help="""skip resulting n-grams that are subsets of any
                             longer n-grams found (default: {})""".format(
            DEFAULT_INCLUDE_SUBGRAMS
        ),
    )
    ...
    parser.add_argument(
        ...
        choices=[SORT_N, SORT_LENGTH, SORT_ALPHA],
        help="""how to sort final output, by n-gram length, text
                             length, or sentence alphabetically (default: {})""".format(
            DEFAULT_SORT
        ),
    )
    ...
    if args.max_words:
        ...
        print(
            "Setting max words to {} based on longest sentence found".format(max_words)
        )
    ...
    for i in range(args.min_words, max_words + 1):
        ...
        if args.zero_results > 0 and count == 0:
            ...
            if zero_results >= args.zero_results:
                print(
                    "Got zero results for {} consecutive n-gram lengths, continuing".format(
                        args.zero_results
                    )
                )
                break
    ...
    lengths_sentences = [
        (n, re.sub(pattern, r"\1", " ".join(m))) for (n, m) in all_matches
    ]
    lengths_sentences = [
        (n, len(sentence), sentence) for (n, sentence) in lengths_sentences
    ]
    ...
    if args.sort == SORT_N:
        ...
        lengths_sentences = sorted(
            lengths_sentences, key=itemgetter(1, 2), reverse=True
        )
